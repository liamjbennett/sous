...
func NewCompileTarget(pack *Pack) *CompileTarget {
	return &CompileTarget{NewJavaTarget("compile", pack)}
}
...
func (t *CompileTarget) Desc() string {
	return "The Java compile target generates a single jar file"
}

// Checking a Java project always passes.
...
func (t *CompileTarget) DockerRun(c *core.Context) *docker.Run {
	...
	// TODO: if project is a multi-module build then cd to the root to build
...
func (t *CompileTarget) artifactPath(c *core.Context) string {
	return fmt.Sprintf("%s/%s.tar.gz", t.artifactDir(c), t.artifactName(c)) //TODO: need to create this tar.gz from the jar
}
